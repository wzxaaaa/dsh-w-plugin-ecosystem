...
export const name = 'route-primer-router-bootstrap'
...
export const W_PERSONA_SECTION = 'deployment:persona'

export function standardSections(assembled) {
  const source = Array.isArray(assembled?.sections) ? assembled.sections : []
  const planSection = source.find(section => /plan/i.test(section?.name ?? ''))
  const customPersona = source.find(section => section?.name === W_PERSONA_SECTION)
  return [
...
export function apply(ctx, config = {}) {
  ...
  function effectiveMode(session) {
    ...
    const liveText = firstUserText.get(session.id)
    return liveText === undefined ? sessionMode(session) : classifyTask(liveText)
  }
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    const mode = effectiveMode(session)
    ...
    if (routerMode === 'standard') {
      persona = RL_PERSONA
      sections = standardSections(assembled)
  ...
  ctx.on('session/event', (session, event) => {
    ...
    try {
      target.inbox.append('next-step', {
        ...
        role: 'user',
        source: { kind: 'plugin', plugin: 'dsh-w-route-primer' },
  ...
  registerTool({
    ...
    execute(args) {
      ...
      const current = effectiveMode(session)
  ...
  registerTool({
    ...
    async execute(args) {
      ...
      let customPersona = ''
      try {
        const manager = ctx.get('personaManager')
        const state = typeof manager?.getState === 'function' ? await manager.getState() : undefined
        if (typeof state?.current === 'string') customPersona = state.current.trim()
      } catch { /* w-persona is optional for isolated calls */ }
      const system = customPersona === '' ? persona : `${persona}\n\n${customPersona}`
  ...
  ctx.on('session/disposed', (session) => {
    ...
    firstUserText.delete(session.id)
  })
}
